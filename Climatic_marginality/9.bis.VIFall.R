...
write.csv2(VIF,file=paste0(Resp,Allcode[i],"_All.Vif.csv"),row.names = F) # Table for the paper with coefficient
...
colnames(VIF)[1] <- paste0(Allcode[i]," Variables")
VIF[,2] <- round(VIF[,2],2)
write.csv2(VIF,file=paste0(Resp,Allcode[i],"_All.Vif.csv"),row.names = F,col.names = T, append = T) # Table for the paper with coefficient
#write.table(c(Allcode[i],VIFSTEP), sep=",", file=paste0(Resp,"_All.Vifstep.csv",row.names = F),append = T) # Table for the paper with coefficient
}
