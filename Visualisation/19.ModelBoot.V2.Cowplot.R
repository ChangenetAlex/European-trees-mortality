...
## Bootstrap function 
ModelBoot.All <- function(x,N = N,Ncat = Ncat,LvL = LvL, CAT = CAT, nBoot = nBoot,Yportion=Yportion, saveboot = saveboot,nCoeur=nCoeur){
  A <- Extraction(x)
  ...
  message(paste0("Species is ",CODE," with model ",mod))
  ...
  yMAX <- max(c(Means_Bootpred0,Means_Bootpred2))+max(c(SD_Bootpred0,SD_Bootpred2)) # Here is the maximum values when removing one category (need to modify the number)
  ...
  {p1 <- p1 + scale_color_manual("",values = c("black", "red", "blue"),labels=(c("Average level","High level","Low level")))+ #change legend when one cat is removed
    scale_shape_manual("", values=c(1,2,3),labels=(c("Average level","High level","Low level")))
  }else p1 <- p1 + scale_color_manual("",values = c("black", "red", "blue"),labels=(c("Core","Trailing Edge","Leading Edge")))+ #change legend when one cat is removed
    scale_shape_manual("", values=c(1,2,3),labels=(c("Core","Trailing Edge","Leading Edge")))
  if (x$family$family=="binomial"){
  ...
  p1 <- p1 + coord_cartesian(ylim=c(0,yMAX),expand = T)+
    ...
    theme(text = element_text(face="bold"),legend.position = c(0.5,0.8),legend.direction ="horizontal", #legend.position = "none",          
  ...
  print(p1)
  assign(paste0("p",CODE,"_",eval(mod),"_",seuil,"_",Variation,"_",VariationCAT),p1,envir = .GlobalEnv) # added to save the figure object because it is really long
  
  if (saveboot==T){
    ggsave(filename = paste0("/home/achangenet/SynologyDrive/FUNDIV - NFI - Europe/our-data/species/Results.all/Species/",CODE,"_",eval(mod),"_",seuil,"_",Variation,"_",VariationCAT,".600.png"),plot = p1, width = 12.37, height = 7.04, dpi=600,units = "in")
...
### Meme fonction moins LE

ModelBoot.RE <- function(x,N = N,Ncat = Ncat,LvL = LvL, CAT = CAT, nBoot = nBoot,Yportion=Yportion, saveboot = saveboot,nCoeur=nCoeur){
  A <- Extraction(x)
  Variation = Extraction(x)[N]
  VariationCAT = Extraction(x)[Ncat]
  Resp <- paste0(getCall(x)[2])
  ...
  # Legend levels depend upon the used variable
  if (is.numeric(data[,VariationCAT]))
  {p1 <- p1 + scale_color_manual("",values = c("black", "red", "red"),labels=(c("Average level","High level","Low level")))+ #change legend when one cat is removed
    scale_shape_manual("", values=c(1,2,3),labels=(c("Average level","High level","Low level")))
  }else p1 <- p1 + scale_color_manual("",values = c("black", "red", "red"),labels=(c("Core","Trailing Edge","Trailing Edge")))+ #change legend when one cat is removed
    scale_shape_manual("", values=c(1,2,3),labels=(c("Core","Trailing Edge","Trailing Edge")))
  if (x$family$family=="binomial"){
    #setwd(dir = paste0("/home/achangenet/Documents/FUNDIV - NFI - Europe/our-data/species/",CODE,"/CLIMAP/Models/binomial/",deparse(substitute(x)),"/")) # All models
    #setwd(dir = paste0("/home/achangenet/Documents/FUNDIV - NFI - Europe/our-data/species/Results.all/Species/",CODE,"/",deparse(substitute(x)),"/")) # One model
    p1 <- p1 + labs(title=paste0(VariationCAT," : ",Variation), y=paste0(CODE,": Mortality occurence probability"), x=Variation)
...
#### Remove TE and keep LE

ModelBoot.LE <- function(x,N = N,Ncat = Ncat,LvL = LvL, CAT = CAT, nBoot = nBoot,Yportion=Yportion, saveboot = saveboot,nCoeur=nCoeur){
  A <- Extraction(x)
  Variation = Extraction(x)[N]
  VariationCAT = Extraction(x)[Ncat]
  Resp <- paste0(getCall(x)[2])
  ...
  # Legend levels depend upon the used variable
  if (is.numeric(data[,VariationCAT]))
  {p1 <- p1 + scale_color_manual("",values = c("black", "blue", "blue"),labels=(c("Average level","High level","Low level")))+ #change legend when one cat is removed
    scale_shape_manual("", values=c(1,2,3),labels=(c("Average level","High level","Low level")))
  }else p1 <- p1 + scale_color_manual("",values = c("black", "blue", "blue"),labels=(c("Core","Leading Edge","Leading Edge")))+ #change legend when one cat is removed
    scale_shape_manual("", values=c(1,2,3),labels=(c("Core","Leading Edge","Leading Edge")))
  if (x$family$family=="binomial"){
    #setwd(dir = paste0("/home/achangenet/Documents/FUNDIV - NFI - Europe/our-data/species/",CODE,"/CLIMAP/Models/binomial/",deparse(substitute(x)),"/")) # All models
    #setwd(dir = paste0("/home/achangenet/Documents/FUNDIV - NFI - Europe/our-data/species/Results.all/Species/",CODE,"/",deparse(substitute(x)),"/")) # One model
    p1 <- p1 + labs(title=paste0(VariationCAT," : ",Variation), y=paste0(CODE,": Mortality occurence probability"), x=Variation)
...
## Bootstrap function 
ModelBoot.r.All <- function(x,N = N,Ncat = Ncat,LvL = LvL, CAT = CAT, nBoot = nBoot,Yportion=Yportion, saveboot = saveboot,nCoeur=nCoeur){
  A <- Extraction(x)
  Variation = Extraction(x)[N]
  VariationCAT = Extraction(x)[Ncat]
  Resp <- paste0(getCall(x)[2])
  ...
  # Legend levels depend upon the used variable
  if (is.numeric(data[,VariationCAT]))
  {p1 <- p1 + scale_color_manual("",values = c("black", "blue", "red"),labels=(c("Average level","High level","Low level")))+ #change legend when one cat is removed
    scale_shape_manual("", values=c(1,2,3),labels=(c("Average level","High level","Low level")))
  }else p1 <- p1 + scale_color_manual("",values = c("black", "blue", "red"),labels=(c("Core","Leading Edge","Trailing Edge")))+ #change legend when one cat is removed
    scale_shape_manual("", values=c(1,2,3),labels=(c("Core","Leading Edge","Trailing Edge")))
  if (x$family$family=="binomial"){
    #setwd(dir = paste0("/home/achangenet/Documents/FUNDIV - NFI - Europe/our-data/species/",CODE,"/CLIMAP/Models/binomial/",deparse(substitute(x)),"/")) # All models
    #setwd(dir = paste0("/home/achangenet/Documents/FUNDIV - NFI - Europe/our-data/species/Results.all/Species/",CODE,"/",deparse(substitute(x)),"/")) # One model
    p1 <- p1 + labs(title=paste0(VariationCAT," : ",Variation), y=paste0(CODE,": Mortality occurence probability"), x=Variation)
...
### Meme fonction moins LE and keep RE

ModelBoot.r.RE <- function(x,N = N,Ncat = Ncat,LvL = LvL, CAT = CAT, nBoot = nBoot,Yportion=Yportion, saveboot = saveboot,nCoeur=nCoeur){
  A <- Extraction(x)
  Variation = Extraction(x)[N]
  VariationCAT = Extraction(x)[Ncat]
  Resp <- paste0(getCall(x)[2])
...
ModelBoot.r.LE <- function(x,N = N,Ncat = Ncat,LvL = LvL, CAT = CAT, nBoot = nBoot,Yportion=Yportion, saveboot = saveboot,nCoeur=nCoeur){
  A <- Extraction(x)
  Variation = Extraction(x)[N]
  VariationCAT = Extraction(x)[Ncat]
  Resp <- paste0(getCall(x)[2])
