...
for (i in c(1:20)){
  ...
  pobs <- ggplot() + theme(panel.background = element_rect(fill="white", colour="black", size=3, 
    ...
    geom_point(data=A3, aes(x=x, y = y),col="gray90",size=0.001)+ #instead of Tile 
    scale_y_continuous(expand = c(0.02,0),limits = c(36,70)) +
    scale_x_continuous(expand = c(0.02,0),limits = c(-10,32)) +
    coord_fixed(1.3,expand=T) +  
    geom_polygon(data = europe1, aes(x=long, y = lat,group=group),fill=NA,col="black") + #gray 60 & gray 60
    ...
    #xlim(-10,32)+
    labs(title=paste0(code[i]," - BIN"), y=paste0("Latitude"), x="Longitude")+
  ...
  xrange <- ggplot_build(pobs)$layout$panel_params[[1]] #NEW
  xrange <- unlist(xrange$x.range) #NEW
  
  yrange <- unlist(ggplot_build(pobs)$layout$panel_params[[1]][8])
  yrange <- ggplot_build(pobs)$layout$panel_params[[1]] #NEW
  yrange <- unlist(yrange$y.range) #NEW
  ...
  save_plot(filename = paste0("~/SynologyDrive/FUNDIV - NFI - Europe/Redaction/Paper1/Figure3/Ag.Inset",code[i],".png"),plot = pallV2,base_width = 5, base_height = 5, dpi = 300 ,units = "in",nrow=1,ncol=1)
  saveRDS(pallV2,file = paste0("~/SynologyDrive/FUNDIV - NFI - Europe/Redaction/Paper1/Figure3/Ag.Inset",code[i],".rds"))
  # assign(paste0("pall",Allcode[i]),pallV2,envir = .GlobalEnv)
  rm("x")
}


A <- list.files(paste0("~/SynologyDrive/FUNDIV - NFI - Europe/Redaction/Paper1/Figure3"),pattern = "Ag.Inset.*rds$",full.names = T)
...
save_plot(filename = paste0("~/SynologyDrive/FUNDIV - NFI - Europe/Redaction/Paper1/Figure3/Ag.Prediction.Mort.bin.Part1bis150.tiff"),
          plot = pall,base_width = 5, dpi = 150, base_height = 5,units = "in",nrow=4,ncol=3)
...
#pall
save_plot(filename = paste0("~/SynologyDrive/FUNDIV - NFI - Europe/Redaction/Paper1/Figure3/Ag.Prediction.Mort.bin.Part2bis.pdf"),
          plot = pall,base_width = 5, base_height = 5, dpi = 600 ,units = "in",nrow=3,ncol=3)
#save_plot(filename = paste0("~/SynologyDrive/FUNDIV - NFI - Europe/Redaction/Paper1/Figure3/Ag.Prediction.Mort.bin.Part2.pdf"),
#          plot = pall,base_width = 5, base_height = 5, dpi = 300 ,units = "in",nrow=3,ncol=3)
