...
if (Allcode[i]%in%c("PINPIN","PINNIG","QUEPET","QUEROB","PINPINA","POPTRE","ALNGLU","QUEPYR","QUESUB")){
  ...
  pobs <- ggplot() + theme(panel.background = element_rect(fill="white", colour="black", size=3, 
    ...
    scale_fill_manual(values = c("darkred","chocolate1","yellow"),name="Mortality occurence \n probability",labels = c("High (>Q3)", "Medium \n(Q1 - Q3)","Low (<Q1)")) +
  ...
  save_plot(filename = paste0("~/SynologyDrive/FUNDIV - NFI - Europe/Redaction/Paper1/Figure3/Inset",Allcode[i],".png"),plot = pallV2,base_width = 5, base_height = 5, dpi = 300 ,units = "in",nrow=1,ncol=1)
  saveRDS(pallV2,file = paste0("~/SynologyDrive/FUNDIV - NFI - Europe/Redaction/Paper1/Figure3/Inset",Allcode[i],".rds"))
